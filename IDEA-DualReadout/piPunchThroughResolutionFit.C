 ...
 */
AnalysisResults analyzeFile(const std::string& input_file, double energy, double Ks, double Kc, double Chi, double Ks_crystal, double Kc_crystal, double Ethreshold) {
	...
	// Crystal section
        ...
        float eventTotalE = ((eventTotalS_cal - Chi*eventTotalC_cal) / (1-Chi)) + eventCrystalTotalS_cal;

	if(eventCrystalTotalS_cal < Ethreshold){
 ...
 */
int piPunchThroughResolutionFit() {
    ...
    const double Ethreshold = 0.5; // [GeV] crystal S energy threshold to select events
    ...
    filesToAnalyze[20.0] = "pilinearity/IDEA_o2_v01_phi0p5_theta0p5_20gev.root";
    filesToAnalyze[40.0] = "pilinearity/IDEA_o2_v01_phi0p5_theta0p5_40gev.root";
    filesToAnalyze[60.0] = "pilinearity/IDEA_o2_v01_phi0p5_theta0p5_60gev.root";
    filesToAnalyze[80.0] = "pilinearity/IDEA_o2_v01_phi0p5_theta0p5_80gev.root";
    ...
    std::cout << "Crystal energy threshold [GeV] : " << Ethreshold << std::endl;
    ...
    for (const auto& pair : filesToAnalyze) {
        ...
        AnalysisResults results = analyzeFile(filename, energy, CALIBRATION_CONSTANT_S, CALIBRATION_CONSTANT_C, CALIBRATION_CONSTANT_Chi, CALIBRATION_CRYSTAL_S, CALIBRATION_CRYSTAL_C, Ethreshold);
    ...
    if (pointIndex > 0) {
        std::cout << "\nSaving Calibrated Graphs, Fits, and Histograms to 'HadEnergyScanCalibratedResults.root'..." << std::endl;
        
        gStyle->SetOptFit(0); // Turn off global fit stats box
        TFile *outFile = new TFile("HadEnergyScanCalibratedResults.root", "RECREATE");
	...
	// --- NEW: S and C Combined Resolution Plot ---
        ...
        outFile->Close();
        std::cout << "Analysis results saved to 'HadEnergyScanCalibratedResults.root'." << std::endl;
